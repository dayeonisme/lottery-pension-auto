...
```bash
gcloud compute addresses create existing-service-ip \
  --region=us-west1
```
...
```bash
gcloud compute addresses describe existing-service-ip --region=us-west1 --format='get(address)'
```
...
```bash
gcloud compute instances create cinelog-vm \
...
cinelog-vm    <ZONE>  e2-micro      ...  RUNNING
```
...
접속 성공 시 프롬프트: `ubuntu@cinelog-vm:~$`
...
```bash
gcloud compute scp \
  <local-existing-service-root>/config/service_account.json \
  ubuntu@cinelog-vm:/home/ubuntu/lottery_auto/config/service_account.json \
  --zone=<ZONE>
```
...
```bash
gcloud compute scp \
  <local-existing-service-root>/data/lotto645_purchases.json \
  <local-existing-service-root>/data/pension720_purchases.json \
  <local-existing-service-root>/data/last_run.json \
  ubuntu@cinelog-vm:/home/ubuntu/lottery_auto/data/ \
  --zone=<ZONE>
```
...
```bash
gcloud compute ssh ubuntu@cinelog-vm --zone=<ZONE> --command="ls ~/lottery_auto/config/ ~/lottery_auto/data/"
```
...
sed -i 's|<local-existing-service-root>|/home/ubuntu/lottery_auto|g' \
...
```bash
grep -c '<local-username>' config/n8n_lotto645_workflow.json config/n8n_pension720_workflow.json
```
